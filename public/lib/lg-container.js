class Container {
  ...
  capturePageSnapshot() {
    // html2canvas can't parse Tailwind v4's oklch/oklab colors, so we draw
    // the page background gradient directly onto a canvas instead.
    ...
    // Mirror the page's linear-gradient(135deg, #edf2f7, #f0ecf7, #f7f0ec)
    const grad = ctx.createLinearGradient(0, 0, w, h)
    grad.addColorStop(0,   '#edf2f7')
    grad.addColorStop(0.5, '#f0ecf7')
    grad.addColorStop(1,   '#f7f0ec')
    ctx.fillStyle = grad
